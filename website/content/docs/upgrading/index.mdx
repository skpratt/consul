...
4. Once all the server agents are upgraded, begin a rollout of client agents following
   the same process.
   Upgrade Envoy proxies (e.g., sidecars, gateways) associated with each client agent
   to a [compatible Envoy version](/docs/connect/proxies/envoy#supported-versions)
   before restarting the client agent with version B.
