...
- `acl` ((#acl)) - This object allows a number of sub-keys to be set which
  ...
  - `tokens` ((#acl_tokens)) - This object holds all of the configured
    ...
    - `default` ((#acl_tokens_default)) - When provided, the agent will
      use this token when making requests to the Consul servers.
      Clients can [override this token on a per-request basis](/api-docs#authentication).
      When not overridden, the empty token, which maps to the 'anonymous' ACL token,
      is used.
...
- `acl_token` ((#acl_token_legacy)) - **Deprecated in Consul 1.4.0. See
  ...
  the agent will use this token when making requests to the Consul servers.
  Clients can [override this token on a per-request basis](/api-docs#authentication).
  When not overridden, the empty token, which maps to the 'anonymous' ACL
  policy, is used.
